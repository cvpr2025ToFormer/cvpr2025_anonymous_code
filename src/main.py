...
import utility
from model_v3.toformer import ToFormer
from summary.dsummary import DepthSummary
from metric.dmetric import DepthMetric
...
def train(gpu, args):
    ...
    if gpu == 0:
        writer_train = DepthSummary(args.save_dir, 'train', args,
                                               loss.loss_name, metric.metric_name)
        writer_val = DepthSummary(args.save_dir, 'val', args,
                                             loss.loss_name, metric.metric_name)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch, sample in enumerate(loader_train):
            ...
            loss_sum, loss_val = loss(sample, output, epoch)
        ...
        if gpu == 0:
            ...
            if epoch % 10 == 0 or epoch == args.epochs or epoch >= args.epochs - 5:
...
def test(args):
    ...
    dep = torch.rand(1, 1, 480, 640).cuda()
    pc = torch.rand(1, 8, 2000).cuda()
    K = torch.rand(1, 4).cuda()
    input = {'rgb': rgb, 'dep': dep, 'pc': pc, 'K': K}

    MACs, params = profile(net_stat, inputs=(input,))
    print("GFLOPs: {}".format((MACs * 2) / 1000 ** 3))
    ...
    if args.pretrain is not None:
        ...
        # key_m, key_u = net.load_state_dict(checkpoint, strict=False)
    ...
    writer_test = DepthSummary(args.save_dir, 'test', args, None, metric.metric_name)
    ...
    print("Total time: {}, Num sample: {}".format(t_total1-t_total0, num_sample))
    print("AVG time: {}".format((t_total1 - t_total0)/num_sample))
