    ...
    def save(self, epoch, idx, sample, output):
        with torch.no_grad():
            if self.args.save_result_only:
                ...
                # Parse data
                feat_init = output['pred_init'].detach()
                ...
                dep = cv2.dilate(dep, kernal, iterations=3)
                pred = pred[0, 0, :, :].data.cpu().numpy()
                pred_gray = pred
                ...
                path_save_gt = '{}/06_gt_min{:.3f}_max{:.3f}.png'.format(self.path_output, gt.min(), gt.max())
